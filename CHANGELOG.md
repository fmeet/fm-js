...
-   Use packages.facemeeting.com for Olm
...
-   consume trailing slash when creating FM Client in HS and IS urls
...
-   Use the FM C/S API v2 with r0 prefix
...
-   `MatrixClient.getAvatarUrlForMember` has been removed and replaced with
    ...
    ```javascript
    // before
    var url = client.getAvatarUrlForMember(
        member,
        width,
        height,
        resize,
...
-   `MatrixClient.getAvatarUrlForRoom` has been removed and replaced with
    ...
    ```javascript
    // before
    var url = client.getAvatarUrlForRoom(
        room,
        width,
        height,
        resize,
...
-   Added support for muting local video and audio via the new methods
    ...
    `MatrixCall.isLocalVideoMuted()` and `FM.setLocalVideoMuted(muted)`.
...
-   Added infrastructure for supporting End-to-End encryption. E2E is _NOT_
...
Crypto infrastructure (crypto is _NOT_ available in this version):
...
-   `MatrixCall`
-   `WebStorageStore` - _WIP; unstable_
-   `WebStorageSessionStore` - _WIP; unstable_
...
-   `"Room"` events will now be emitted when the Room has _finished_ being
